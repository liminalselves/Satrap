...
class Embedding(_EmbeddingBase[OpenAI]):
        ...
        for i in range(0, total_count, batch_size):  # 每次处理 batch_size 个文本
            ...
            try:
                ...
                if any(not vector for vector in embeddings):
                    logger.warning(
                        f"[Embedding] 第 {batch_num}/{total_batches} 批次包含空结果"
                    )
                ...
                logger.debug(
                    f"[Embedding] 第 {batch_num}/{total_batches} 批次处理完成，共 {len(embeddings)} 条"
                )
                ...
                if not self.suppress_error:
                    raise
                logger.error(
                    f"[Embedding] 第 {batch_num}/{total_batches} 批次 API 错误: {e}"
                )
                if self.return_false:
                    return False
                ...
                if not self.suppress_error:
                    raise
                logger.error(
                    f"[Embedding] 第 {batch_num}/{total_batches} 批次调用过程发生未知异常: {e}"
                )
                if self.return_false:
                    return False
                all_embeddings.extend([[] for _ in batch_texts])
    ...
    def _create_client(self, api_key: str, timeout: int) -> OpenAI:
        return OpenAI(api_key=api_key, base_url=self.base_url, timeout=timeout)
