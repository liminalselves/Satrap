...
from .sync_summary import _SyncSummary
from .utils import _ContextRuntimeState, _messages_domain, add_tool_message
...
class ContextManager(_SyncSummary):
    """对话上下文管理器"""
    ...
    def add_tool_message(self, tool_call_id: str, tool_result: dict[str, Any] | str) -> None:
        """
        添加工具调用结果消息
    ...
    def del_message(self, index: int):
        ...
        if 0 <= index < len(self._messages) or -len(self._messages) <= index < 0:
            ...
            logger.error(
                f"[上下文管理] 删除消息失败: 索引 {index} 超出范围, ID: {self.conversation_id}"
            )
        self._mark_dirty()
        self._sync()
    ...
    def export_json(self, file_path: str):
        ...
        try:
            ...
            logger.info(
                f"[上下文管理] 成功导出 json 文件: {file_path}, ID: {self.conversation_id}"
            )

        except Exception as e:
            logger.error(
                f"[上下文管理] 导出 json 文件失败: {e}, ID: {self.conversation_id}"
            )
    ...
    def _commit_turn_messages(self, messages: list[dict[str, Any]]) -> None:
        ...
        try:
            if not self.keep_in_memory:
                self.save_context(raise_on_error=True)
