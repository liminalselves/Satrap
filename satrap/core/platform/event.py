...
class MessageSession:
    ...
    def __str__(self) -> str:
        ...
        返回:
        - str:  format: platform_name:message_type:session_id
        """
    ...
    def from_str(cls, s: str) -> MessageSession:
        ...
        返回:
        - MessageSession: 从 "platform_name:message_type:session_id" 格式字符串反序列化
        """
        parts = s.split(":", 2)
        if len(parts) < 3:
            logger.error(
...
class MessageChain:
    ...
    def components(self) -> list[BaseMessageComponent]:
        ...
        返回:
        - list[BaseMessageComponent]: 所有消息组件
        """
    ...
    def to_dict_list(self) -> list[dict[str, Any]]:
        ...
        返回:
        - list[dict[str, Any]]: 转为字典列表, 用于序列化或传递给下层
        """
...
class MessageEventResult:
    ...
    def stop_event(self) -> MessageEventResult:
        ...
        返回:
        - MessageEventResult: 标记事件处理停止 (后续处理器不再执行)
        """
...
class MessageEvent:
    ...
    def unified_msg_origin(self) -> str:
        ...
        返回:
        - str: 统一消息来源标识, 格式: platform_name:message_type:session_id
        """
    ...
    def get_message_type(self) -> str:
        ...
        返回:
        - str: 消息类型 (如 friend_message / group_message)
        """
    ...
    def get_extra(self, key: str | None = None, default: Any = None) -> Any:
        ...
        返回:
        - Any: 自定义扩展字段; key 为 None 时返回全部扩展字段
        """
        if key is None:
            return self._extras
    ...
    def get_result(self) -> MessageEventResult | None:
        ...
        返回:
        - MessageEventResult | None: 事件处理结果
        """
        return self._result
    ...
    def plain_result(self, text: str) -> MessageEventResult:
        ...
        返回:
        - MessageEventResult: 快速创建纯文本事件结果
        """
    ...
    def image_result(self, url_or_path: str) -> MessageEventResult:
        ...
        返回:
        - MessageEventResult: 快速创建图片事件结果 (自动识别 URL 或本地路径)
        """
    ...
    def chain_result(self, chain: list[BaseMessageComponent]) -> MessageEventResult:
        ...
        返回:
        - MessageEventResult: 从消息组件列表创建事件结果
        """
    ...
    async def get_group(self, group_id: str | None = None) -> Any:
        ...
        返回:
        - Any: 群聊信息; 不传 group_id 时自动使用消息中的群 ID
        """
        if isinstance(self.adapter, PlatformAdapter):
            try:
    ...
    def has_send_operation(self) -> bool:
        ...
        返回:
        - bool: 是否已执行过发送操作(如已通过 content_callback 发送)
        """
    ...
    async def process_buffer(self, buffer: str, pattern: re.Pattern[str]) -> str:
        ...
        返回:
        - str: 按正则模式逐步从缓冲区提取并发送匹配内容, 每次发送后等待 1.5 秒
        """
        while True:
