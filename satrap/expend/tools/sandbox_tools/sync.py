...
from .utils import SyncExecutionAuthorizer
...
class CodeSandboxTool(Tool):
    ...
    def __init__(
        self,
        sandbox: CodeSandbox,
        execution_authorizer: SyncExecutionAuthorizer | None = None,
    ):
        """
        初始化 CodeSandboxTool
    ...
    def execute(
        ...
        try:
            ...
            if operation in ("run", "run_file"):
                ...
                if not self._authorize_execution(description):
                    return {"error": "代码执行需要用户明确批准, 当前请求已拒绝"}
            return execute_operation(self.sandbox, operation, code, path)
