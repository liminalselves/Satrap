...
def _base_id(session_id: str) -> str:
    ...
    返回:
    - str: 提取 session_id 的固定前缀(去掉尾部随机段), 兼容新旧格式
    """
    parts = session_id.split(":")
    if len(parts) >= 4:
        return ":".join(parts[:3])
