...
- 文件和目录只读工具访问工作区内路径
- Shell 门禁判定为只读且仅访问工作区内路径的命令

以下情况需要审批或直接拒绝:

- 文件写入、编辑、删除和所有 Shell 写入命令
...
`ask_user` 工具建议模型提供 2-3 个推荐选项 (options 参数, 编号展示), 用户可输入序号选择。执行统一走 `shell` (共享本机全环境, 非安全边界), 文件读写走文件工具。Shell 只读命令可直接执行, 所有写入或高风险命令均需审批; 工作区外路径、动态环境路径和嵌套 shell 会提升风险或直接拒绝。`code_sandbox` 仅限制工作目录, 不构成操作系统隔离, 因此 `run` 和 `run_file` 没有明确用户授权时默认拒绝。沙箱路径越界和删除沙箱根目录始终拒绝。
